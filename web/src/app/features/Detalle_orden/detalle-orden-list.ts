...
import { DetalleOrdenService } from '../../core/services/detalle_orden.service';
import { DetalleOrdenRead } from '../../models/api.models';
import { shortId } from '../../shared/ids';

type DetalleDialogResult = { id_orden: string; id_producto: string; cantidad: number };
...
export class DetalleOrdenDialogComponent {
  ...
  private readonly ref: MatDialogRef<DetalleOrdenDialogComponent, DetalleDialogResult> =
    inject(MatDialogRef);
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    cantidad: [this.data.row?.cantidad ?? 1, [Validators.required, Validators.min(1)]],
  });
...
export class DetalleOrdenListComponent implements OnInit {
  ...
  nuevo(): void {
    this.dialog
      ...
      .afterClosed()
      .subscribe((result: DetalleDialogResult | undefined) => {
        if (!result) return;
        this.svc
  ...
  eliminar(row: DetalleOrdenRead): void {
    if (!confirm(`¿Eliminar detalle ${shortId(row.id_detalle)}?`)) return;
